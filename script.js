...
// ----------------------------------------------------------------------
// ASSISTENTE VIRTUAL (NOVO CÓDIGO - INTEGRADO COM LÓGICA GEMINI SEGU   RO)
// ----------------------------------------------------------------------
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (assistantButton) assistantButton.addEventListener('click', toggleChat);
    ...
    function appendMessage(text, type) {
        ...
        // Permite formatação simples (como negrito) e quebras de linha
    ...
    async function sendMessage() {
        ...
        try {
            // ** CHAMADA SEGURA PARA O BACKEND DE IA **
            const assistantResponse = await getAssistantResponseSecure(userQuery); 
            ...
            removeTypingIndicator();
            // Esta mensagem aparecerá se o servidor backend falhar ou não estiver no ar
            appendMessage('Desculpe, houve um erro de comunicação com o servidor de Inteligência Artificial. Tente novamente ou me pergunte sobre as funcionalidades da Agenda Online (Tarefas, Premium, Calendário).', 'assistant-message');
    ...
    // ** ------------------------------------------------------------------ **
    // ** FUNÇÃO CENTRAL DE RESPOSTA (CHAMADA SEGURA À API DO GEMINI/BACKEND) **
    // ** ------------------------------------------------------------------ **
    async function getAssistantResponseSecure(userQuery) {
...
if (lowerQuery.includes('premium') || lowerQuery.includes('pagar')) {
    ...
    return "Se for sua primeira vez usando o app, acesse o **Tutorial Interativo** na tela inicial. Ele mostra passo a passo como criar tarefas, configurar lembretes e usar o calendário.";
        ...
        try {
            ...
            if (!response.ok) {
                // Se o servidor backend falhar
                throw new Error(`Erro do servidor: ${response.status} ${response.statusText}`);
            }

            const data = await response.json();
            
            // O backend deve retornar um JSON com o texto da IA na propriedade 'text'
            const aiResponseText = data.text; 
            
            return aiResponseText || "O servidor de IA respondeu, mas não consegui ler a mensagem. Tente reformular sua pergunta.";

        } catch (error) {
            // Resposta de fallback quando o servidor de IA está inacessível
            console.error("Erro na comunicação com o backend de IA:", error);
